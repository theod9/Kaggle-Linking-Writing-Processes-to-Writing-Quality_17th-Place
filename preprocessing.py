...
def transform_and_clean_data(X):
    """
    Scale the features, replace NaNs with zero, and clean column names.
    ...
    X_tran.columns = X.columns

    # Replace NaNs with zero
    X_tran = X_tran.fillna(0)

    # Clean the column names to ensure they are valid Python identifiers
...
def clean_column_names(df):
    ...
    df.columns = [pattern.sub('_', col) if pattern.search(col) else col for col in df.columns]
    return df
